...
public function store(Request $request)
{
    ...
    $anneeMois = date('Ym'); // Ex. : "202503"
    $idUtilisateur = auth()->user()->id; // ID de l’utilisateur connecté
    $lettreAleatoire = chr(rand(65, 90)); // Lettre aléatoire de A à Z
    $matricule = "ETU-{$anneeMois}-{$lettreAleatoire}-{$idUtilisateur}";

    // Ajouter le matricule aux données validées
